...
#define RES_NAME_LEN_LIM 50 // not include '\0'
...
int check_include(const char *, long * loc, char res [][RES_NAME_LEN_LIM + 1]);
...
// set variables and open files
	...
	strncpy(outfile_name, str_arg[1], RES_NAME_LEN_LIM);
...
// open the include_rsc_file
	...
	char res_name[RES_NUM_LIM][RES_NAME_LEN_LIM + 1];
...
extern bool process_complete; // default false
int check_include(const char * in, long pos[], char res_name [][RES_NAME_LEN_LIM + 1])
{
	...
	// check resources files' name and write position
		...
		if (in[index] == '<')
		{
			...
			int index_2;
			for (index_2 = 0; in[index] != '\0' && in[index] != '\n' && index_2 < RES_NAME_LEN_LIM; index_2++, index++) // copy res file name; index_2 have go after file name
			{
				if (in[index] != '>')
				{
